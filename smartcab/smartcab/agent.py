...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            # self.epsilon = self.epsilon - 0.05 # decay function
            self.epsilon = self.epsilon * self.a # new decay function
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            if random.uniform(0,1) <= self.epsilon: # choose randome w.p. epsilon
                action = random.choice(self.valid_actions)
            else:    # choose an action with highest Q-value
                actions = [action for action, re in self.Q[self.state].items() if re == self.get_maxQ(state)]
...
def run():
    ...
    sim.run(tolerance=0.01, n_test=25)
